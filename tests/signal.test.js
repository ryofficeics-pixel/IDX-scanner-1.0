...
function signalAt(iso, overrides = {}, market = { ihsgChangePct:0.2 }) {
  const now = new Date(iso);
  return generateSignal(stock({ timestamp:now.toISOString(), ...overrides }), market, sessionContext(now), histories(now));
}
