...
function taRSI(values, period) {
  if (values.length < period + 1) return null;
  const r = TI.RSI.calculate({ values, period });
  return r.length ? r[r.length - 1] : null;
}

function taMACD(values) {
  if (values.length < 33) return { macd: null, signal: null, histogram: null };
  const r = TI.MACD.calculate({ values, fastPeriod: 12, slowPeriod: 26, signalPeriod: 9 });
...
function taBollinger(values, period, stdDev) {
  if (values.length < period) return { upper: null, middle: null, lower: null, pctB: null };
  const r = TI.BollingerBands.calculate({ values, period, stdDev });
  const last = r.length ? r[r.length - 1] : null;
  return last ? { upper: last.upper, middle: last.middle, lower: last.lower, pctB: last.pb } : { upper: null, middle: null, lower: null, pctB: null };
}

function taATR(candles, period) {
  ...
  const r = TI.ATR.calculate({ high: highs, low: lows, close: closes, period });
  return r.length ? r[r.length - 1] : null;
}

function taStochastic(candles, period, signalPeriod) {
  const highs = candles.map((c) => Number(c.high)).filter(Number.isFinite);
  const lows = candles.map((c) => Number(c.low)).filter(Number.isFinite);
  const closes = candles.map((c) => Number(c.close)).filter(Number.isFinite);
  if (highs.length < period + signalPeriod || lows.length < period + signalPeriod || closes.length < period + signalPeriod) return { k: null, d: null };
  const r = TI.Stochastic.calculate({ high: highs, low: lows, close: closes, period, signalPeriod });
  const last = r.length ? r[r.length - 1] : null;
  return last ? { k: last.k, d: last.d } : { k: null, d: null };
}

function calculateIndicators(stock, daily = [], intraday = [], session = {}) {
  const { completedDaily, partitionIntraday } = require('../market/historyContext');
  daily = completedDaily(daily, session.now || new Date());
  intraday = partitionIntraday(intraday, session.now || new Date()).current;
  ...
  const vwap = vwapDen > 0 ? intraday.reduce((a, c) => a + ((Number(c.high ?? c.close) + Number(c.low ?? c.close) + Number(c.close)) / 3 * (Number(c.volume) || 0)), 0) / vwapDen : null;
  ...
  const freshnessScore = stock.timestampSource === 'fetch' || timestampMs > referenceNow + 60000 || dataAgeMinutes == null ? 40 : dataAgeMinutes <= 5 ? 100 : dataAgeMinutes <= 15 ? 82 : dataAgeMinutes <= 30 ? 55 : 25;
  ...
  // ---------------------------------------------------------------------
  // TECHNICAL ANALYSIS INDICATORS (via technicalindicators library)
  //   - RSI daily/short for overbought/oversold detection
  //   - MACD for trend confirmation
  //   - Bollinger %B for volatility & mean-reversion
  //   - ATR for volatility sizing
  //   - Stochastic for short-term momentum turns
  const dailyClosesForTA = dailyCloses.length >= 50 ? dailyCloses : null;
  const dailyCandlesForTA = daily.length >= 15 && daily.every((c) => c.high > 0 && c.low > 0 && c.close > 0) ? daily : null;
  const rsiDaily = dailyClosesForTA ? taRSI(dailyClosesForTA, 14) : null;
  const rsiShort = dailyClosesForTA ? taRSI(dailyClosesForTA, 7) : null;
  const macd = dailyClosesForTA ? taMACD(dailyClosesForTA) : { macd: null, signal: null, histogram: null };
  const bb = dailyClosesForTA ? taBollinger(dailyClosesForTA, 20, 2) : { upper: null, middle: null, lower: null, pctB: null };
  const atr = dailyCandlesForTA ? taATR(dailyCandlesForTA, 14) : null;
  const atrPct = atr != null && lastPrice > 0 ? (atr / lastPrice) * 100 : null;
  const stoch = dailyCandlesForTA ? taStochastic(dailyCandlesForTA, 14, 3) : { k: null, d: null };

  // TA-enhanced scores
  const rsiOversold = rsiDaily != null && rsiDaily < 30 ? 20 : rsiDaily != null && rsiDaily < 40 ? 10 : 0;
  const rsiOverbought = rsiDaily != null && rsiDaily > 70 ? -15 : rsiDaily != null && rsiDaily > 60 ? -5 : 0;
  const macdBullish = macd.histogram != null && macd.histogram > 0
    && macd.macd != null && macd.signal != null && macd.macd > macd.signal ? 10 : 0;
  const bollingerSqueeze = bb.upper != null && bb.lower != null && lastPrice > 0
    ? clamp(100 - ((bb.upper - bb.lower) / lastPrice) * 100) : 50;
  const bollingerPB = bb.pctB != null ? clamp(bb.pctB * 100) : 50;
  const stochBullish = stoch.k != null && stoch.d != null && stoch.k > stoch.d && stoch.k < 80 ? 10 : 0;
  return {
    changePct,
    dataAgeMinutes,
    rangePosition,
