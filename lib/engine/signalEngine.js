...
function generateSignal(stock, marketContext = {}, sessionContext = {}, histories = {}) {
  ...
  const dq = dataQualityScore(stock);
  const indicators = calculateIndicators(stock, histories.daily || [], histories.intraday || [], sessionContext);
  ...
  let score = changeScore * 0.20
    + volumeScore * 0.20
    + projectedVolumeScore * 0.15
    + indicators.intradayTrendScore * 0.15
    + rangePositionScore * 0.10
    + indicators.breakoutScore * 0.10
    + indicators.liquidityScore * 0.07
    + marketScore * 0.03
    - risk.penalty;
  ...
  else if (score >= 85 && dq.score >= 80 && confidence >= 75 && risk.level !== 'HIGH' && indicators.changePct >= 1 && indicators.changePct <= 8 && indicators.projectedVolRatio >= 1.5 && indicators.rangePosition >= 0.70 && indicators.liquidityScore >= 60 && indicators.intradayTrendScore >= 65 && (marketContext.ihsgChangePct ?? 0) > -0.5) action = 'STRONG_BUY';
  else if (score >= 70 && dq.score >= 70 && risk.level !== 'HIGH') action = 'BUY';
  ...
  const morningDirect = inMorningBuyWindow(now) && score >= 75 && dq.score >= 75 && indicators.changePct >= 0.8 && indicators.changePct <= 5 && indicators.projectedVolRatio >= 1.8 && indicators.rangePosition >= 0.75 && indicators.liquidityScore >= 60 && risk.level !== 'HIGH' && indicators.lateFadeScore <= 35;
  const afternoonDirect = inAfternoonBuyWindow(now) && score >= 75 && dq.score >= 75 && indicators.rangePosition >= 0.70 && indicators.projectedVolRatio >= 1.3 && indicators.intradayTrendScore >= 70 && indicators.lateFadeScore <= 35 && risk.level !== 'HIGH';
