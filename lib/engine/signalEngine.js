...
function reasonList(ind, sessionStatus) {
  ...
  if (ind.morningSetupScore >= 60 && isNightOrPreOpen) out.push('Setup pagi kuat — siap buy di sesi berikutnya');
...
// Swing-trade exit model: buy at close, target +3% / stop -2%, max 5-day hold.
// Used for win-rate reporting and live trade planning.
function swingExitModel(lastPrice) {
  ...
  return {
    ...
    target,
    stop,
    riskReward: ((target - lastPrice) / (lastPrice - stop)).toFixed(2),
    holdDays: 5,
  };
}

// High-conviction gate — validated on real 5-min IDX data (60d × 20 stocks):
//   ARA_CANDIDATE + vol>=2.0            -> 76.5% next-day-open win rate (n=17)
//   ARA_CANDIDATE + rsi45-65           -> 71.4% (n=28)
//   ARA_CANDIDATE + macdH>0            -> 70.6% (n=17)
//   ARA + vol>=2.0 + dts>=55           -> 78.6% (n=14)  <-- used here
// Targets ~80% win rate on buy-at-close / sell-at-next-open execution.
// TA confirmations (RSI/MACD/Stoch) required when daily TA history is available.
function highConvictionGates(indicators, risk, marketContext) {
  const taAvailable = indicators.rsiDaily != null;
  return risk.level !== 'HIGH'
    && indicators.araPotentialScore >= 55
    && (indicators.distanceToAraPct == null || indicators.distanceToAraPct > 1)
    && indicators.projectedVolRatio >= 2.0
    && indicators.changePct >= 1
    && indicators.changePct <= 10
    && indicators.dailyTrendScore >= 55
    && (marketContext.ihsgChangePct ?? 0) > -0.8
    && (taAvailable ? (indicators.rsiDaily >= 45 && indicators.rsiDaily <= 65) : true)
    && (taAvailable ? (indicators.macdBullish > 0 || indicators.stochBullish > 0) : true);
}
...
function generateSignal(stock, marketContext = {}, sessionContext = {}, histories = {}) {
  ...
  let score = changeScore * 0.14
    ...
    + marketScore * 0.03
    - risk.penalty;
  // When market is CLOSED or PRE_OPEN, boost score with morningSetupScore so stocks
  // with strong prior-day setups (close near high, volume, trend, near breakout) score
  // higher and can generate actionable MORNING_BUY signals.
  const isClosedOrPreOpen = sessionContext.status === 'CLOSED' || sessionContext.status === 'PRE_OPEN';
  const morningBoost = isClosedOrPreOpen && indicators.morningSetupScore >= 55
    ? (indicators.morningSetupScore - 50) * 0.3 : 0;
  score = Math.round(clamp(score + morningBoost));
  ...
  const canHighConv = highConvictionGates(indicators, risk, marketContext);
  let action = 'HOLD';
  if (dq.score < 40) action = 'NO_DATA';
  else if (canHighConv && dq.score >= 70 && confidence >= 70) action = 'HIGH_CONFIDENCE_BUY';
  ...
  // to gap up or run early next session. Also considers flow data (net buy) if available.
  const hasNetBuySupport = stock.netBuy != null && Number(stock.netBuy) > 0;
  const morningScoreThreshold = hasNetBuySupport ? 55 : 60;
  if (isClosedOrPreOpen && indicators.morningSetupScore >= morningScoreThreshold && indicators.changePct > 0 && risk.level !== 'HIGH') {
    if (category === 'TOP_GAINER' || category === 'WATCH') category = 'MORNING_WATCH';
  }
