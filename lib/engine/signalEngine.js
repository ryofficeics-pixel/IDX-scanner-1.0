...
function reasonList(ind) {
  ...
  if (ind.araPotentialScore >= 70) out.push(`Potensi ARA tinggi (${ind.araProgressPct.toFixed(0)}% dari batas, sisa ruang ${ind.distanceToAraPct != null ? ind.distanceToAraPct.toFixed(1) : '?'}%)`);
...
function buyGates(indicators, risk, marketContext) {
  return risk.level !== 'HIGH'
    ...
    // FIX: was <= 8, which permanently excluded any stock already up >8% intraday from BUY --
    // exactly the early stage of a stock heading toward a 20-30% day. Raised to a proxy ceiling
    // near typical IDX auto-reject-up (ARA) territory. lateFadeScore/volatilityControlScore/
    // freshnessScore below already filter out an exhausted/faded spike, so this doesn't remove
    // risk control, just stops blocking genuine momentum. NOTE: real ARA bands vary by price
    // tier on IDX (this codebase doesn't have that table) -- tighten this if you add it.
...
function strongBuyGates(indicators, risk, marketContext) {
  return buyGates(indicators, risk, marketContext)
    ...
    // FIX: was <= 6, tighter than the already-too-tight BUY ceiling. Raised for the same
    // reason as above -- the quality gates (freshnessScore>=82, gapControlScore>=70,
    // volatilityControlScore>=65) are the real exhaustion filter here, not a flat %-move cap.
...
function generateSignal(stock, marketContext = {}, sessionContext = {}, histories = {}) {
  ...
  // Surface strong ARA (auto-reject-up) candidates: already showing real strength with volume/
  // range confirmation but not yet at the daily ceiling. See indicators.js for araPotentialScore.
  const araCandidate = indicators.araPotentialScore >= 70 && risk.level !== 'HIGH' && (indicators.distanceToAraPct == null || indicators.distanceToAraPct > 1);
