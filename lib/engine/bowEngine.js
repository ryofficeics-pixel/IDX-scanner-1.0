...
function generateBowSignal(stock, marketContext = {}, histories = {}) {
  ...
  const gateForeign = stock.netBuy == null || num(stock.netBuy) > 0;
  ...
  const fs = foreignScore(num(stock.netBuy));
  ...
  if (num(stock.netBuy) > 0) {
    const nb = num(stock.netBuy);
    reasons.push(`Foreign net buy Rp${(Math.abs(nb)/1e9).toFixed(2)}B — asing akumulasi`);
  }
  ...
  if (gateVolume && gateForeign && gateMA50Prox) {
    ...
    if (num(stock.netBuy) > 1e9) boomFactors.push('foreign akumulasi besar');
