...
const { completedDaily, partitionIntraday, verifiedQuote } = require('../market/historyContext');
const { setupFeatures } = require('./setupEngine');
const { triggerFeatures } = require('./triggerEngine');
const { sessionContext } = require('../market/idxSession');
...
function buildEntry(lastPrice, support, invalidation = support * 0.95) {
  if (!(lastPrice > 0) || !(support > 0)) return null;
  const stopLoss = invalidation;
  if (!(stopLoss > 0 && stopLoss < lastPrice)) return null;
...
function relativeStrengthScore(stockReturn3M, ihsgReturn3M) {
  if (stockReturn3M == null || ihsgReturn3M == null) return { raw:45, weighted:6.75, reason:'RS vs IHSG tidak dapat dihitung' };
  const excessReturn = stockReturn3M - ihsgReturn3M;
  const raw = clamp(50 + excessReturn * 3);
  return { raw, weighted:raw * 0.15, reason:`Return relatif IHSG ${excessReturn.toFixed(1)} poin persentase` };
}

function generateBowSignal(stock, marketContext = {}, histories = {}) {
  stock = verifiedQuote(stock, partitionIntraday(histories.intraday || [], histories.now || new Date()).current);
  const daily = completedDaily(histories.daily || [], histories.now || new Date());
  ...
  const gateCloseAboveMA200 = ma200 != null && lastPrice > ma200;
  ...
  const changePct = stock.previousClose > 0 ? pct(lastPrice, stock.previousClose) : null;
  const priceNotMoved = changePct != null && Math.abs(changePct) <= 3 && drawdownPct >= 3;
  if (priceNotMoved) reasons.push(`Perubahan harian ${changePct.toFixed(1)}%, harga masih berada di bawah puncak sebelumnya`);
  ...
  const setup = setupFeatures(stock, daily, histories.ihsgDaily || []);
  const distance = (level) => level > 0 ? Math.abs(pct(lastPrice, level)) : Infinity;
  const trendHealthy = daily.length >= 80 && ma20 > 0 && ma50 > 0 && ma20 >= ma50 && (ma200 == null || lastPrice > ma200);
  const quietPullback = setup.volumeDryUpRatio != null && setup.volumeDryUpRatio < 1 && drawdownPct >= 2;
  const oldResistance = daily.length >= 25 ? Math.max(...daily.slice(-25, -5).map((row) => num(row.high) ?? row.close)) : null;
  const brokeResistance = oldResistance > 0 && closes.slice(-5).some((close) => close > oldResistance);
  const families = [
    ['MA20_PULLBACK', trendHealthy && distance(ma20) <= 2.5 && drawdownPct >= 2, ma20],
    ['MA50_PULLBACK', gateCloseAboveMA200 && distance(ma50) <= 3 && drawdownPct >= 3, ma50],
    ['BREAKOUT_RETEST', trendHealthy && brokeResistance && distance(oldResistance) <= 2 && lastPrice >= oldResistance * 0.995, oldResistance],
    ['CONTRACTION_PULLBACK', trendHealthy && quietPullback && setup.volatilityCompressionScore >= 55 && distance(ma20) <= 3, ma20],
    ['FLOW_PULLBACK', trendHealthy && quietPullback && nbDisplay > 0 && distance(ma20) <= 3, ma20],
    ['FAILED_BREAKDOWN_RECLAIM', trendHealthy && stock.dayLow > 0 && stock.dayLow < ma20 && lastPrice > ma20 && stock.previousClose <= ma20, ma20],
  ];
  const invalid = (!trendHealthy && !gateCloseAboveMA200) || drawdownPct > 25 || !(lastPrice * todayVolume >= 5e8) || changePct > 4 || changePct < -5;
  const patterns = families.filter(([, matches]) => matches && !invalid).map(([name, , support]) => ({
    name, support, invalidation:support * 0.97,
    score:Math.round(clamp(55 + setup.priceTightnessScore * 0.10 + setup.volumeDryUpScore * 0.10 + setup.flowScore * 0.10 - distance(support) * 2)),
  })).sort((a, b) => b.score - a.score);
  const pattern = patterns[0];
  const legacyEligible = !rejectReasons.length && score >= 55;
  const now = histories.now || new Date();
  const intraday = partitionIntraday(histories.intraday || [], now).current;
  const trigger = triggerFeatures(stock, intraday, { ...sessionContext(now), now });
  const patternConfirmed = Boolean(pattern && (trigger.vwapReclaim || trigger.microBreakout)
    && trigger.volumeAcceleration >= 1.3 && trigger.priceVelocity > 0 && !trigger.failedBreakout
    && trigger.falseBreakoutPenalty < 35 && stock.lastPrice > pattern.support);
  if (pattern && pattern.score >= 65 && pattern.score > score) {
    score = pattern.score;
    rejectReasons.length = 0;
    reasons.length = 0;
    reasons.push(`${pattern.name}: harga ${distance(pattern.support).toFixed(1)}% dari support ${Math.round(pattern.support)}`);
    if (quietPullback) reasons.push('Volume pada pullback menurun dibanding rata-rata');
    if (nbDisplay > 0) reasons.push('Foreign net buy positif tersedia');
    reasons.push(`Setup batal bila harga turun di bawah ${Math.round(pattern.invalidation)}`);
  }
  boomScore = Math.max(boomScore, !rejectReasons.length && pattern ? setup.setupScore : 0);
  const selectedEntry = pattern && !rejectReasons.length ? buildEntry(lastPrice, pattern.support, pattern.invalidation) : entryModel;
  const quoteAgeMs = stock.timestamp ? new Date(now) - new Date(stock.timestamp) : NaN;
  const quoteVerified = stock.timestampSource !== 'fetch' && Number.isFinite(quoteAgeMs) && quoteAgeMs >= -60000 && quoteAgeMs <= 900000;
  const pendingRecovery = Boolean(pattern && !legacyEligible && !patternConfirmed);
  const watchOnly = pendingRecovery || (!quoteVerified && !rejectReasons.length);
  if (pendingRecovery) reasons.push('Setup menunggu reclaim atau breakout intraday dengan konfirmasi volume');
  if (!quoteVerified) risks.push('Waktu harga belum terverifikasi atau harga sudah kedaluwarsa; tunggu harga baru');
  ...
  return {
    ...
    lastPrice,
    previousClose:num(stock.previousClose),
    changePct,
    marketVolume:todayVolume,
    avgVolume20,
    tradedValue:lastPrice != null && todayVolume != null ? lastPrice * todayVolume : null,
    score,
    category: watchOnly ? 'Watchlist' : bowCategory(score),
    verdict: watchOnly ? 'Watchlist' : bowCategory(score),
    action: watchOnly ? 'WATCH' : score >= 55 && !rejectReasons.length && selectedEntry ? 'BOW_BUY' : 'AVOID',
    drawdownPct,
    rsiDaily: rsiVal,
    ma50dist: ms.pct,
    ...
    entry: selectedEntry,
    strategyFamily:pattern?.name || 'LEGACY_MA50_OVERSOLD',
    patterns,
    patternConfirmed,
    quoteVerified,
    ...
    preMarketPlan: !watchOnly && !rejectReasons.length && boomScore >= 50 ? {
      strategy: 'buy on weakness sebelum breakout',
      entryOpen: Math.round(lastPrice * 1.01),
      entryLimit: selectedEntry?.aggressiveEntry ?? null,
      stopLoss: selectedEntry?.stopLoss ?? null,
      target: selectedEntry?.target1 ?? null,
...
function generateMorningSignal(stock, marketContext = {}, histories = {}) {
  const bow = generateBowSignal(stock, marketContext, histories);
  const daily = completedDaily(histories.daily || [], histories.now || new Date());
  const setup = setupFeatures(stock, daily, histories.ihsgDaily || []);
  const change = stock.previousClose > 0 ? pct(stock.lastPrice, stock.previousClose) : null;
  const morningEligible = setup.setupHistoryAvailable && setup.setupScore >= 60 && stock.lastPrice * stock.volume >= 5e8
    && change != null && Math.abs(change) <= 4 && setup.breakoutProximityScore >= 60;
  if (morningEligible && (bow.morningBoom?.score || 0) < setup.setupScore) {
    bow.morningBoom = { score:setup.setupScore, category:'Setup', factors:[`Kompresi ${setup.volatilityCompressionScore}`, `Kedekatan resistance ${setup.breakoutProximityScore}`] };
    bow.setupScore = setup.setupScore;
    if (bow.action !== 'BOW_BUY') {
      bow.action = 'WATCH'; bow.category = 'MORNING_WATCH'; bow.verdict = 'Watchlist';
      bow.preMarketPlan = null;
    }
  }
  return { ...bow, morningEligible };
}

module.exports = { generateBowSignal, generateMorningSignal };
