...
function dataQualityScore(stockData, now = new Date()) {
  const warnings = [];
  let score = 100;
  ...
  const referenceNow = new Date(now).getTime();
  const ageMs = stockData.timestamp ? referenceNow - new Date(stockData.timestamp).getTime() : Infinity;
