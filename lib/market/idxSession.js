...
function wibDate(now = new Date()) {
  return new Date(new Date(now).getTime() + 7 * 3600000);
}
...
function inMorningBuyWindow(now = new Date()) {
  ...
  return sessionContext(now).status === 'MORNING' && m >= 9 * 60 && m <= 10 * 60 + 30;
}

function inAfternoonBuyWindow(now = new Date()) {
  const m = minutes(wibDate(now));
  return ['AFTERNOON', 'PRE_CLOSE'].includes(sessionContext(now).status) && m >= 14 * 60 + 30 && m <= 15 * 60 + 49 + 59 / 60;
}
