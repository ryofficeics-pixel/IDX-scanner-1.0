...
async function getBatchQuotes(symbols) {
  ...
  const missing = clean.filter((symbol) => !quotes[symbol]).slice(0, 60);
  ...
  const result = { quotes, ihsg:quotes['^JKSE'] || null, failedSymbols:[...new Set(failedSymbols)], warnings, provider:'yahoo-finance' };
