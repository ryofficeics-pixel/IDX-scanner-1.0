...
function normalizeStockRow(row) {
  ...
  // Bid/offer quantities are not executed broker flow.
  const brokerBuy  = num(row?.BrokerBuy ?? row?.brokerBuy ?? null);
  const brokerSell = num(row?.BrokerSell ?? row?.brokerSell ?? null);
  ...
  return {
    ...
    lastPrice:close, previousClose:prev, dayOpen:open, dayHigh:high, dayLow:low,
    volume, value, freq,
    ...
    tradeDate: String(row?.Date ?? row?.date ?? '').slice(0, 10) || null,
  };
}
...
async function getBatchStockSummary(options = {}) {
  ...
  try {
    ...
    // FIX: `warnings` was never declared anywhere in this module -- this threw a second,
    // unhandled ReferenceError on every fetch failure instead of degrading gracefully to
    // Yahoo-only flow as this comment claims. Now actually non-fatal.
    console.warn(`IDX stock summary fetch failed: ${err.message}`);
  }
